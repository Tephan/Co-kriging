...
dato <- read.csv('C:\\Users\\tepha\\Desktop\\Se馻les\\KRIGING\\S2-d1\\S2-d1.csv')
...
v = variogram(Tetha~1, data=dato)
#Beta Method 0
#m <- vgm(psill=0.0033,model="Gau",range=1.2, nugget=0.00025)
#Alfa Method 0
#m <- vgm(psill=0.0065,model="Gau",range=2, nugget=0.00001)
#Delta Method 2
#m <- vgm(psill=0.018,model="Gau",range=1, nugget=0.0001)
#Tetha method 0.9
m <- vgm(psill=0.01,model="Gau",range=2, nugget=0.0004)
#Gamma method 2
#m <- vgm(psill=0.026,model="Gau",range=1, nugget=0.00001)

#plot(v$dist, v$gamma,  xaxs="i", yaxs="i")
plot(v, model=m)
#Variograma ajustado
FittedModel <- fit.variogram(v,
                             model=m,fit.method = 0.9)

plot(v, model=FittedModel)
gridded(dato.grid) = ~X+Y

#Creaci贸n del Krige.
a <- krige(Tetha~1, dato,
