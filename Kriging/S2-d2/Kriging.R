...
dato <- read.csv('C:\\Users\\tepha\\Desktop\\Se馻les\\KRIGING\\S2-d2\\S2-d2.csv')
...
v = variogram(Delta~1, data=dato)
#Beta Method 1
#m <- vgm(psill=0.00325,model="Exp",range=1.2, nugget=0.00001)
#Alfa Method 7.5
#m <- vgm(psill=0.0019,model="Exp",range=1.5, nugget=0.000001)
#Delta Method 0
m <- vgm(psill=0.025,model="Mat",range=1.2, nugget=0.0001)
#Tetha method 0.9
#m <- vgm(psill=0.0175,model="Gau",range=2, nugget=0.0004)
#Gamma method 0
#m <- vgm(psill=0.022,model="Gau",range=1, nugget=0.0004)

#plot(v$dist, v$gamma,  xaxs="i", yaxs="i")
plot(v, model=m)
#Variograma ajustado
FittedModel <- fit.variogram(v,
                             model=m,fit.method = 0)

plot(v, model=FittedModel)
gridded(dato.grid) = ~X+Y

#Creaci贸n del Krige.
a <- krige(Delta~1, dato,
