...
dato <- read.csv('C:\\Users\\tepha\\Desktop\\Se馻les\\KRIGING\\S5-d2\\S5-d2.csv')
...
#Beta Method 0
#m <- vgm(psill=0.00142,model="Exp",range=2, nugget=0.000015)
#Alfa Method 0
#m <- vgm(psill=0.0009,model="Exp",range=2, nugget=0.000001)
#Delta Method 0
#m <- vgm(psill=0.053,model="Exp",range=1.5, nugget=0.0002)
#Tetha method 0
#m <- vgm(psill=0.0041,model="Exp",range=1, nugget=0.00005)
#Gamma method 0
m <- vgm(psill=0.00304,model="Gau",range=2, nugget=0.00022)

#plot(v$dist, v$gamma,  xaxs="i", yaxs="i")
plot(v, model=m)
#Variograma ajustado
FittedModel <- fit.variogram(v,
                             model=m,fit.method = 0.5)
