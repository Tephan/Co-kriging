...
dato <- read.csv('C:\\Users\\tepha\\Desktop\\Se馻les\\KRIGING\\S4-d2\\S4-d2.csv')
...
#Beta Method 2
#m <- vgm(psill=0.0048,model="Exp",range=1.2, nugget=0.00001)
#Alfa Method 1
m <- vgm(psill=0.00078,model="Exp",range=2, nugget=0.000001)
#Delta Method 2
#m <- vgm(psill=0.048,model="Exp",range=1.5, nugget=0.0002)
#Tetha method 2
#m <- vgm(psill=0.01,model="Exp",range=1, nugget=0.00005)
#Gamma method 2
#m <- vgm(psill=0.008,model="Sph",range=2, nugget=0.0004)

#plot(v$dist, v$gamma,  xaxs="i", yaxs="i")
plot(v, model=m)
#Variograma ajustado
FittedModel <- fit.variogram(v,
                             model=m,fit.method = 1)
